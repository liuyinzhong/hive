...
- 库存余额页按当前仓库、SKU 编码、批号、仅正库存、排序和 VXE 远端导出配置创建导出任务，按钮权限为 `erp:inventoryBalance:export`；导出列为空时由后端拒绝创建任务。
- 开发任务页按当前任务标题、项目、版本、状态和排序创建导出任务，按钮权限为 `dev:task:export`。
- 开发任务页复用 VXE Table 的远端导出面板传递文件名、工作表名、勾选列、表头和源数据选项；操作列不会进入导出列清单。后端按白名单生成文件，未提供列配置时沿用历史固定列。
- 登录日志页按当前用户名、事件类型、状态、IP、日期、排序和 VXE 远端导出配置创建导出任务，按钮权限为 `system:loginLog:export`；导出列为空时由后端拒绝创建任务，任务完成后在下载中心查看进度和下载文件。
