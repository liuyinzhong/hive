...
本目录记录医生管理和医生排班的页面入口、按钮展示、表单联动和交互约束。业务规则正文位于后端仓库 `hive-admin-go/business-docs/medical`。
...
3. 阅读 [医生管理 UI](./doctor-ui.md) 或 [医生排班 UI](./schedule-ui.md)。
...
- 页面状态判断和按钮隐藏不能代替后端校验。
- 患者、挂号和挂号费尚未迁入本目录，相关任务继续核对后端根 `CONTEXT.md` 和当前代码。
