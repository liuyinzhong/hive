...
- 具体测试用例：
  - 内部工具包的单元测试位于 internal/* 下的 __tests__ 目录
  - 核心UI能力的单元测试位于 packages/* 下的 __tests__ 目录
- 端到端测试：playground/__tests__/e2e 下的 spec 文件
...
### 测试文件组织结构与命名规范
- 组织结构
  - 内部工具包测试：internal/<pkg>/src/__tests__/
  - 核心能力测试：packages/@core/<pkg>/__tests__/
  - 端到端测试：playground/__tests__/e2e/
- 命名规范
  - 测试文件以 .test.ts 结尾
