...
## 项目结构
该仓库采用Monorepo结构，使用pnpm进行包管理与workspace组织，结合Turbo进行任务编排与缓存优化。核心应用位于apps目录下，包含多个UI框架适配版本；内部工具与配置位于internal目录；开发脚本位于scripts目录；文档站点位于docs目录；根级配置文件定义了统一的脚本命令、引擎要求与工作区范围。

```mermaid
graph TB
Root["根项目<br/>package.json / pnpm-workspace.yaml / turbo.json"] --> Apps["应用层<br/>apps/web-*"]
Root --> Internal["内部配置与工具<br/>internal/*"]
Root --> Scripts["开发脚本<br/>scripts/*"]
Root --> Docs["文档站点<br/>docs/*"]
Apps --> WebAntd["@vben/web-antd<br/>vite.config.ts"]
Apps --> WebEle["@vben/web-ele<br/>vite.config.ts"]
Apps --> WebNaive["@vben/web-naive<br/>vite.config.ts"]
Apps --> WebTDesign["@vben/web-tdesign<br/>vite.config.ts"]
Internal --> ViteCfg["vite配置库<br/>internal/vite-config"]
Internal --> LintCfg["lint配置库<br/>internal/lint-configs/*"]
Scripts --> TurboRun["turbo-run<br/>多应用开发入口"]
...
- 应用配置：各web应用通过各自的vite.config.ts加载统一的vite配置库，集中处理代理、插件与开发服务器参数。
- 质量工具链：ESLint与Oxlint配置由内部lint-configs提供，配合格式化与拼写检查工具，形成统一的代码规范。
- 开发脚本：turbo-run与vsh作为统一入口，简化多应用开发与质量检查流程。
...
## 架构总览
下图展示了从开发命令到具体应用启动、代理与预览的整体流程，体现多应用开发与统一配置的协作关系。
...
participant Mock as "后端Mock服务"
Dev->>Turbo : 执行开发命令
Turbo->>App : 启动指定应用开发模式
App->>Vite : 加载统一vite配置
...
- 启动开发服务器
  - 根级脚本提供统一入口，turbo-run负责协调多应用开发。
  - 各应用通过自身的vite脚本启动开发模式，加载统一的vite配置库。
- 环境变量与模式
  - 通过loadAndConvertEnv加载.env与环境变量，确保开发模式正确注入。
  - 模式切换（development/production/analyze）在各应用脚本中定义。
...
TurboRun --> AppDev["应用启动<br/>@vben/web-antd dev"]
AppDev --> LoadCfg["加载统一配置<br/>@vben/vite-config"]
...
- 插件与中间件
  - 统一的vite配置库提供插件注册与选项扩展能力，确保各应用行为一致。
- WebSocket与代理
...
### 开发效率工具与自动化脚本
- 多应用开发入口
  - turbo-run提供统一开发入口，简化多应用同时启动与切换。
- 质量检查工具
...
## 结论
本指南基于仓库现有配置，系统梳理了Vben Admin的开发工作流：从环境搭建、依赖管理、开发服务器启动，到热重载与调试、代码规范与质量保障、以及效率工具与自动化脚本。建议在实际开发中结合各应用的vite配置与统一的vite配置库，确保开发体验的一致性与可维护性。
