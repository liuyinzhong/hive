...
- 包含范围
  - internal/*、packages/*、apps/*、scripts/*、docs、playground 等，确保所有子包与应用被纳入工作区。
...
- 全局依赖
  - 监听 pnpm-lock.yaml、.env.*local、tsconfig*、内部工具与脚本源码变更，触发或失效缓存。
- 全局环境
  - 暴露 NODE_ENV，保证各任务使用一致的运行时环境。
- 任务定义
  - build：按拓扑顺序依赖上游包（^build），输出 dist/**、dist.zip、.vitepress/dist.*，支持增量构建与缓存命中。
...
- 应用层配置
  ...
  - 应用 package.json：通过 imports 映射 #/* 到 src/*，简化路径书写；依赖统一走 workspace:* 或 catalog:。
- 内部配置库
