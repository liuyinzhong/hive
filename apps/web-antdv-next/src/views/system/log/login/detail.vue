...
import { getLoginLogDetailApi } from '#/api/system';
...
<template>
  <Drawer
    :footer="false"
    :title="$t('system.log.loginDetail')"
    class="w-[min(800px,90vw)]"
  >
    <Spin :spinning="loading">
      <template v-if="detail">
        <Descriptions bordered :column="2" size="small">
          ...
          <DescriptionsItem :label="$t('system.log.eventType')">
            {{
              $t(
                detail.eventType === 'login'
                  ? 'system.log.login'
                  : 'system.log.logout',
              )
            }}
          ...
          <DescriptionsItem :label="$t('system.log.contentType')">{{
            detail.contentType || '-'
          }}</DescriptionsItem>
